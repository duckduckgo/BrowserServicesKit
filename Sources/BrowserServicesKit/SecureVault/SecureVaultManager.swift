...
extension SecureVaultManager: AutofillSecureVaultDelegate {
    ...
    public func autofillUserScript(_: AutofillUserScript,
        ...
        do {
            ...
            if let passwordManager = passwordManager, passwordManager.isEnabled {
                ...
                delegate?.secureVaultManager(self, promptUserToStoreAutofillData: dataToPrompt, generatedPassword: false, trigger: data.trigger)
                return
            }

            let automaticallySavedCredentials = try storeOrUpdateAutogeneratedCredentials(domain: domain, autofillData: data)
            let generatedPassword = data.credentials?.autogenerated ?? false

            if delegate?.secureVaultManagerShouldAutomaticallyUpdateCredentialsWithoutUsername(self,
                                                                                               generatedPassword: generatedPassword) ?? false {
                try updateExistingCredentialsWithoutUsernameWithSubmittedValues(domain: domain, autofillData: data)
            }

            let dataToPrompt = try existingEntries(for: domain, autofillData: data, automaticallySavedCredentials: automaticallySavedCredentials, generatedPassword: generatedPassword)
            delegate?.secureVaultManager(self, promptUserToStoreAutofillData: dataToPrompt, generatedPassword: data.credentials?.autogenerated ?? false, trigger: data.trigger)
        } catch {
    ...
    func storeOrUpdateAutogeneratedCredentials(domain: String, autofillData: AutofillUserScript.DetectedAutofillData) throws -> Bool {
        let shouldSilentlySaveCredentials = delegate?.secureVaultManagerShouldSilentlySaveGeneratedPassword(self) ?? false
