...
public class SecureVaultManager {
    ...
    private var partialAccountCreated = false
    private let partialAccountsManager = PartialAccountsManager()
...
extension SecureVaultManager: AutofillSecureVaultDelegate {
        ...
        do {
            ...
            if let passwordManager = passwordManager, passwordManager.isEnabled {
                if !shouldSilentlySave {
                    ...
                    autosaveAccount = nil
                    autosaveAccountCreatedInSession = false
                    partialAccountsManager.removePartialAccount(for: domain)
                }
                return
            } else {
    ...
    func storeOrUpdateAutogeneratedCredentials(domain: String, autofillData: AutofillUserScript.DetectedAutofillData) throws {
        ...
        guard autogeneratedCredentials || autofillData.trigger == .partialSave,
    ...
    private func existingCredentials(with autofillData: AutofillUserScript.DetectedAutofillData,
        ...
        let partialAccount = partialAccountsManager.partialAccount(forDomain: domain)
        ...
        guard let accounts = try? vault.accountsFor(domain: domain),
              ...
              var account = accounts.first(where: {
                ...
                let account = partialAccount ?? SecureVaultModels.WebsiteAccount(username: credentials.username ?? "", domain: domain)
        ...
        guard let existingAccountId = account.id,
            ...
            return partialAccount.flatMap {
                SecureVaultModels.WebsiteCredentials(account: $0,
        ...
        if autofillData.hasAutogeneratedCredentials ||
            ...
            // If the previously saved username was empty, default to the submitted one. HERE IS WHERE TO USE PARTIAL DATA
    ...
    private func existingCredentialsInPasswordManager(with autofillData: AutofillUserScript.DetectedAutofillData,
        ...
        guard let existingCredentials = passwordManager.cachedWebsiteCredentialsFor(domain: domain,
                                                                                    username: credentials.username ?? partialAccount?.username ?? "") else {
            // No existing stored credentials found and not auto-generated for username, so return any partial accounts or new entry
            let account = partialAccount ?? SecureVaultModels.WebsiteAccount(username: credentials.username ?? "", domain: domain)
            return SecureVaultModels.WebsiteCredentials(account: account, password: passwordData)
...
final class PartialAccountsManager {
