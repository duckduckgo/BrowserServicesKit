...
extension SecureVaultManager: AutofillSecureVaultDelegate {
    ...
    public func autofillUserScript(_: AutofillUserScript,
        ...
        do {
            ...
            // If a field was auto-generated, keep track of it
            if data.trigger == .emailProtection || data.trigger == .passwordGeneration {
            ...
            if data.trigger == .formSubmission,
               let stringId = autosaveAccount?.id,
                let id = Int64(stringId) {

                let existingCredentials =  try? vault?.websiteCredentialsFor(accountId: id)
                let existingUsername = existingCredentials?.account.username
                let existingPassword =  String(decoding:  existingCredentials?.password ?? Data(), as: UTF8.self)
                ...
                // we should not consider there are autosaved credentials anymore
