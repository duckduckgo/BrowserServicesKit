...
public class SecureVaultManager {
    ...
    private var partialAccount: SecureVaultModels.WebsiteAccount?
...
extension SecureVaultManager: AutofillSecureVaultDelegate {
    ...
    public func autofillUserScript(_: AutofillUserScript,
        ...
        do {
            ...
            if domain != partialAccount?.domain {
                partialAccountCreated = false
                partialAccount = nil
            }
            ...
            case .partialSave:
                partialAccount = SecureVaultModels.WebsiteAccount(username: data.credentials?.username, domain: domain)
                return
            ...
            if !(autogeneratedCredentials || partialAccountCreated) && data.trigger == .formSubmission {
    ...
    func existingEntries(for domain: String,
        ...
        if let partialAccount {
            proposedCredentials = SecureVaultModels.WebsiteCredentials(account: partialAccount,
                                                                       password: autofillData.credentials?.password?.data(using: .utf8))
        } else if let passwordManager = passwordManager, passwordManager.isEnabled {
    ...
    private func existingCredentials(with autofillData: AutofillUserScript.DetectedAutofillData,
        ...
        guard !partialAccountCreated else {
            guard let autosaveAccount else {
                return nil
            }
            return SecureVaultModels.WebsiteCredentials(account: autosaveAccount, password: passwordData)
        }
