...
extension SecureVaultManager: AutofillSecureVaultDelegate {
    ...
    func storeOrUpdateAutogeneratedCredentials(domain: String, autofillData: AutofillUserScript.DetectedAutofillData) throws {
        ...
        } else if let account = accounts.first(where: { $0.username == autogeneratedCredentials.username }) {
            currentAccount = account

        // Create an account to save data while in the form
        } else {
            currentAccount = getAutosaveAccount(vault: vault, username: autogeneratedCredentials.username ?? "",
                                                 password: Data((autogeneratedCredentials.password ?? "").utf8),
                                                 domain: domain)
        }
        ...
        if let id = currentAccount.id {
            ...
            let pwd = autogeneratedCredentials.password ?? ""
            var pwdData: Data
            if pwd == "" {
            ...
            if let username = autogeneratedCredentials.username, username != "" {
    ...
    private func getAutosaveAccount(vault: SecureVault, username: String, password: Data, domain: String) -> SecureVaultModels.WebsiteAccount {
        if let account = autosaveAccount, account.domain == domain {
            return account
        } else {
