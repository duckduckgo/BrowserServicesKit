...
extension SecureVaultManager: AutofillSecureVaultDelegate {
    ...
    func updateExistingAutogeneratedCredentialsWithSubmittedValues(domain: String, autofillData: AutofillUserScript.DetectedAutofillData) throws {
        ...
        guard let autofillCredentials = autofillData.credentials,
              ...
              var existingAccount = (try vault.accountsFor(domain: domain)).first(where: { $0.username == "" }),
        ...
        if existingPasswordData == autofillPasswordData {
            ...
            existingAccount.title = nil // Remove the "Saved Password" title so that Logins+ uses the default title format
