...
public protocol SecureVaultManagerDelegate: SecureVaultErrorReporting {
    ...
    func secureVaultManager(_: SecureVaultManager,
                            promptUserToUseGeneratedPasswordForDomain: String,
...
extension SecureVaultManager: AutofillSecureVaultDelegate {
    ...
    public func autofillUserScript(_: AutofillUserScript,
        ...
        do {
            ...
            var autogenCreds: Bool = false

            if let passwordManager = passwordManager, passwordManager.isEnabled {
                let dataToPrompt = try existingEntries(for: domain, autofillData: data, automaticallySavedCredentials: false, generatedPassword: false)
                delegate?.secureVaultManager(self, promptUserToStoreAutofillData: dataToPrompt, generatedPassword: false)
                return
            }
            ...
            if let credentials = data.credentials,
               ...
               let autoSavedCredentialsId = delegate?.secureVaultManagerAutoSavedCredentialsId(self),
               !autoSavedCredentialsId.isEmpty {
                autogenCreds = true
            }

            let dataToPrompt = try existingEntries(for: domain, autofillData: data, automaticallySavedCredentials: automaticallySavedCredentials, generatedPassword: autogenCreds)
            delegate?.secureVaultManager(self, promptUserToStoreAutofillData: dataToPrompt, generatedPassword: data.credentials?.autogenerated ?? false)
        } catch {
    ...
    public func autofillUserScript(_: AutofillUserScript,
        ...
        delegate?.secureVaultManager(self,
                                     promptUserToUseGeneratedPasswordForDomain: domain,
    ...
    func storeOrUpdateAutogeneratedCredentials(domain: String, autofillData: AutofillUserScript.DetectedAutofillData) throws -> Bool {
        let shouldSilentlySaveCredentials = delegate?.secureVaultManagerShouldAutomaticallySaveGeneratedPassword(self) ?? false

        guard autofillData.hasAutogeneratedPassword,
              let autogeneratedCredentials = autofillData.credentials,
              !(autogeneratedCredentials.username?.isEmpty ?? true) || shouldSilentlySaveCredentials,
            ...
            if let account = accounts.first(where: { $0.username == credentials.username ?? "" }) {
                if let existingAccountID = account.id,
                   ...
                   existingCredentials.password == passwordData {
                    if automaticallySavedCredentials || generatedPassword {
