...
final class DefaultDatabaseProvider: SecureVaultDatabaseProvider {
    ...
    /*
     Should match domain regardless of if the search domain or the saved domains contain ".www".
     Should still be sensitive to other subomains though (e.g. "cnn.com" and edition.cnn.com" should be treated as different)
     */
    func websiteAccountsForDomain(_ domain: String) throws -> [SecureVaultModels.WebsiteAccount] {
        return try db.read {
            let normalizedDomain = normalizedDomainDroppingWWW(domain)
