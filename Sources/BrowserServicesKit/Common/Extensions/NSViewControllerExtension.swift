...
extension NSViewController {
    ...
    public func applyModalWindowStyleIfNeeded() {
    ...
    public func wrappedInWindowController() -> NSWindowController {
        let window = NSWindow(contentViewController: self)
        return NSWindowController(window: window)
    }

    public func beginSheet(_ windowController: NSWindowController) {
        if let windowSheet = windowController.window {
            view.window?.beginSheet(windowSheet)
        }
    }

    public func beginSheet(_ viewController: NSViewController) {
