...
public class WebsiteAutofillUserScript: AutofillUserScript {
    ...
    func showAutofillParent(_ message: UserScriptMessage, _ replyHandler: MessageReplyHandler) {
        ...
        if let jsonData = serializedInputContext.data(using: .utf8) {
            let inputData = try? JSONDecoder().decode(InputData.self, from: jsonData)
            ...
            if let inputType = inputData?.inputType,
               let prefix = inputType.split(separator: ".").first {
                if SecureInputTypes(rawValue: String(prefix)) != nil {
                    ...
                    // Do not require Vault access to generated passwords
                    if ((inputData?.credentials?[0].autogenerated) != nil) {
