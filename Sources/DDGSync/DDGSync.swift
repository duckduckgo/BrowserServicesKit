...
public class DDGSync: DDGSyncing {
    ...
    public weak var dataProvidersProvider: DataProviderProviding?

    /// This is the constructor intended for use by app clients.
    public convenience init(dataProvidersProvider: DataProviderProviding, log: @escaping @autoclosure () -> OSLog = .disabled) {
        let dependencies = ProductionDependencies(baseUrl: Constants.baseUrl, log: log())
        self.init(dataProvidersProvider: dataProvidersProvider, dependencies: dependencies)
    }
    ...
    init(dataProvidersProvider: DataProviderProviding, dependencies: SyncDependencies) {
        self.dataProvidersProvider = dataProvidersProvider
    ...
    private func updateAuthState(startSyncIfNeeded: Bool = true) {
        ...
        if previousState == .inactive && authState != .inactive {
            let providers = dataProvidersProvider?.dataProviders() ?? []
