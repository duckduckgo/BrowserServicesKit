...
class SecureVaultManagerTests: XCTestCase {
    ...
    func testWhenRequestingToStoreCredentials_AndCredentialsDoNotExist_ThenTheDelegateIsPromptedToStoreAutofillData() {
        ...
        let autofillData = AutofillUserScript.DetectedAutofillData(identity: nil, credentials: incomingCredentials, creditCard: nil, trigger: nil)
    ...
    func testWhenRequestingToStoreCredentials_AndCredentialsAreGenerated_AndNoCredentialsAlreadyExist_ThenTheDelegateDoesNotPromptToStoreAutofillData() throws {
    ...
    func testWhenRequestingToStoreCredentials_AndCredentialsAreAutoGenerated_AndCredentialsDoNotExist_ThenAutosaveAccountIsCreated() throws {
        let domain = "domain.com"
        let incomingCredentials = AutofillUserScript.IncomingCredentials(username: "username", password: "password", autogenerated: true)
        let autofillData = AutofillUserScript.DetectedAutofillData(identity: nil, credentials: incomingCredentials, creditCard: nil, trigger: nil)
        manager.autofillUserScript(mockAutofillUserScript, didRequestStoreDataForDomain: domain, data: autofillData)

        XCTAssertEqual(manager.autosaveAccount?.username, "username")
        guard let stringId = manager.autosaveAccount?.id, let id = Int64(stringId) else {
            XCTFail("Missing account Id")
            return
        }
        let credentials = try testVault.websiteCredentialsFor(accountId: id)
        XCTAssertEqual(credentials?.password, Data("password".utf8))
    }

    func testWhenRequestingToStoreCredentials_AndCredentialsAreAutoGenerated_AndCredentialsExist_ThenPasswordIsAutosaved() throws {
        let domain = "domain.com"
        let account = SecureVaultModels.WebsiteAccount(id: "1", title: nil, username: "username", domain: domain, created: Date(), lastUpdated: Date())
        self.mockDatabaseProvider._accounts = [account]
        let credentials = SecureVaultModels.WebsiteCredentials(account: account, password: "password".data(using: .utf8)!)
        ...
        let incomingCredentials = AutofillUserScript.IncomingCredentials(username: "username", password: "anotherPassword", autogenerated: true)
        let autofillData = AutofillUserScript.DetectedAutofillData(identity: nil, credentials: incomingCredentials, creditCard: nil, trigger: nil)

        manager.autofillUserScript(mockAutofillUserScript, didRequestStoreDataForDomain: domain, data: autofillData)

        let storedCredentials = try testVault.websiteCredentialsFor(accountId: 1)
        XCTAssertEqual(storedCredentials?.password, Data("anotherPassword".utf8))
    }

    func testWhenRequestingToStoreCredentials_AndCredentialsAreAutoGenerated_AndCredentialsDoNotExist_ThenUpdatesAreSaved() throws {
        let domain = "domain.com"
        var incomingCredentials = AutofillUserScript.IncomingCredentials(username: "username", password: "password", autogenerated: true)
        var autofillData = AutofillUserScript.DetectedAutofillData(identity: nil, credentials: incomingCredentials, creditCard: nil, trigger: nil)

        manager.autofillUserScript(mockAutofillUserScript, didRequestStoreDataForDomain: domain, data: autofillData)
        XCTAssertEqual(manager.autosaveAccount?.username, "username")

        incomingCredentials = AutofillUserScript.IncomingCredentials(username: "", password: "", autogenerated: true)
        autofillData = AutofillUserScript.DetectedAutofillData(identity: nil, credentials: incomingCredentials, creditCard: nil, trigger: nil)

        // Username and password should not change if empty
        manager.autofillUserScript(mockAutofillUserScript, didRequestStoreDataForDomain: domain, data: autofillData)
        XCTAssertEqual(manager.autosaveAccount?.username, "username")
        ...
        var storedCredentials = try testVault.websiteCredentialsFor(accountId: id)
        XCTAssertEqual(storedCredentials?.password, Data("password".utf8))

        // Username should be updated
        incomingCredentials = AutofillUserScript.IncomingCredentials(username: "anotherUsername", password: "", autogenerated: true)
        autofillData = AutofillUserScript.DetectedAutofillData(identity: nil, credentials: incomingCredentials, creditCard: nil, trigger: nil)

        manager.autofillUserScript(mockAutofillUserScript, didRequestStoreDataForDomain: domain, data: autofillData)
        XCTAssertEqual(manager.autosaveAccount?.username, "anotherUsername")

        // Password should be updated
        incomingCredentials = AutofillUserScript.IncomingCredentials(username: "", password: "anotherPassword", autogenerated: true)
        autofillData = AutofillUserScript.DetectedAutofillData(identity: nil, credentials: incomingCredentials, creditCard: nil, trigger: nil)
        manager.autofillUserScript(mockAutofillUserScript, didRequestStoreDataForDomain: domain, data: autofillData)
        ...
        storedCredentials = try testVault.websiteCredentialsFor(accountId: id)
        XCTAssertEqual(storedCredentials?.password, Data("anotherPassword".utf8))

        // Both should be updated
        incomingCredentials = AutofillUserScript.IncomingCredentials(username: "thirdUsername", password: "thirdPassword", autogenerated: true)
        autofillData = AutofillUserScript.DetectedAutofillData(identity: nil, credentials: incomingCredentials, creditCard: nil, trigger: nil)
        manager.autofillUserScript(mockAutofillUserScript, didRequestStoreDataForDomain: domain, data: autofillData)
        ...
        storedCredentials = try testVault.websiteCredentialsFor(accountId: id)
        XCTAssertEqual(manager.autosaveAccount?.username, "thirdUsername")
        XCTAssertEqual(storedCredentials?.password, Data("thirdPassword".utf8))
