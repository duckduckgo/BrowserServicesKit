...
final class ClickToLoadRulesSplitterTests: XCTestCase {
    ...
    func testShouldFallbackToEmbeddedIfThereAreNoTrackers() {
        // given
        let trackerData = TrackerData(trackers: [:], entities: [:], domains: [:], cnames: nil)
        let rulesList = ContentBlockerRulesList(name: "TrackerDataSet", trackerData: nil, fallbackTrackerData: (trackerData, "embedded"))
    ...
    func testShoulNotdSplitTrackerDataWithoutCTLActions() {
        ...
        let splitRules = splitTrackerDataSet(rawTDS: exampleNonCTLRules, etag: etag)
    ...
    func testShouldSplitTrackerDataWithCTLActions() {
        ...
        guard let splitRules = splitTrackerDataSet(rawTDS: exampleCTLRules, etag: etag) else {
    ...
    private func splitTrackerDataSet(rawTDS: String, etag: String) -> (withoutBlockCTL: ContentBlockerRulesList, withBlockCTL: ContentBlockerRulesList)? {
        let fullTDS = rawTDS.data(using: .utf8)!
        let fullTrackerData = (try? JSONDecoder().decode(TrackerData.self, from: fullTDS))!
        let dataSet = TrackerDataManager.DataSet(tds: fullTrackerData, etag)
