...
final class CredentialsProviderTests: XCTestCase {
    ...
    func testThatPrepareForFirstSyncClearsLastSyncTimestampAndSetsModifiedAtForAllBookmarks() async throws {
        let secureVault = try makeSecureVault()

        try secureVault.inDatabaseTransaction { database in
            try secureVault.storeCredentialsMetadata(domain: "1", username: "1", password: "1", in: database)
            try secureVault.storeCredentialsMetadata(domain: "2", username: "2", password: "2", in: database)
            try secureVault.storeCredentialsMetadata(domain: "3", username: "3", password: "3", in: database)
            try secureVault.storeCredentialsMetadata(domain: "4", username: "4", password: "4", in: database)
        }

        XCTAssertTrue(try secureVault.modifiedWebsiteCredentialsMetadata().isEmpty)

        let date = Date()

        try provider.prepareForFirstSync()

        let modifiedCredentials = try secureVault.modifiedWebsiteCredentialsMetadata()

        XCTAssertEqual(modifiedCredentials.count, 4)
        XCTAssertTrue(modifiedCredentials.allSatisfy { $0.metadata.lastModified! > date })
    }

    // MARK: - Helpers

    func makeSecureVault() throws -> SecureVault {
...
extension SecureVault {
    ...
    func storeCredentialsMetadata(domain: String? = nil, username: String? = nil, password: String? = nil, notes: String? = nil, in database: Database? = nil) throws {
        let passwordData = password.flatMap { $0.data(using: .utf8) }
        let account = SecureVaultModels.WebsiteAccount(username: username, domain: domain, notes: notes)
        let credentials = SecureVaultModels.WebsiteCredentials(account: account, password: passwordData)
        let metadata = SecureVaultModels.SyncableWebsiteCredentialInfo(credentials: credentials, lastModified: nil)
        if let database {
